...
const PointCloudViewer = forwardRef(({ 
  ...
  // 計測ツール
  const measuringToolRef = useRef(null);
  ...
  const initThreeJS = () => {
    ...
    // 計測ツールを初期化
    const measuringTool = new MeasuringTool({
      renderer: renderer,
      camera: camera,
      pointCloud: null // 後で点群が読み込まれたときに設定
    });
    measuringToolRef.current = measuringTool;
  ...
  const toggleMeasurementMode = () => {
    ...
    if (!isMeasurementMode) {
      // 計測モードを開始
      if (measuringToolRef.current && currentPointCloudRef.current) {
        // 点群参照を更新
        measuringToolRef.current.viewer.pointCloud = currentPointCloudRef.current;
        
        // 計測挿入を開始
        const measure = measuringToolRef.current.startInsertion({
          showDistances: true,
        ...
        // 計測完了時のイベントリスナー
        measuringToolRef.current.addEventListener('measurement_finished', (event) => {
          const totalDistance = event.measure.getTotalDistance();
          setMeasurementDistance(totalDistance);
          console.log('計測完了:', totalDistance);
        });
      }
    } else {
      // 計測モードを停止
      if (measuringToolRef.current) {
        measuringToolRef.current.cancelInsertion();
